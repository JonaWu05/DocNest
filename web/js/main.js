...
// 分頁連結對話框：關閉、點遮罩關閉、即時搜尋過濾
document.getElementById("doc-close").addEventListener("click", closeDocPicker);
...
setEnterAppHandler(async (defaultDoc) => {
  ...
  // 根目錄新增鈕：無根目錄寫入權時隱藏（子資料夾的新增鈕另依各節點 writable 控制）
  document.getElementById("new-file-btn").classList.toggle("hidden", !state.canWriteRoot);
  document.getElementById("new-dir-btn").classList.toggle("hidden", !state.canWriteRoot);
  buildTOC();  // 先顯示「未開啟檔案」提示（開檔後會被覆蓋）
  await loadFileTree();
  // 完全沒有可讀內容者：顯示歡迎頁，不自動開檔
  if (!state.hasAccess) {
    previewPane.innerHTML =
      '<div class="empty-state"><i class="fa fa-hand-peace-o"></i>' +
